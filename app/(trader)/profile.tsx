...
import { TradeCategory, ExperienceLevel, PriceRating, Community, Trader } from '@/types';
...
export default function ProfileScreen() {
  ...
  const [serviceAreas, setServiceAreas] = useState<Community[]>(myTraderProfile?.serviceAreas || []);
  ...
  const toggleServiceArea = (area: Community) => {
  ...
  const handleSave = () => {
    if (!businessName || !ownerName || !description || !phone || !email || !yearsInBusiness || serviceAreas.length === 0) {
    ...
    <>
      ...
      <SafeAreaView style={styles.container} edges={['top']}>
        ...
        <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.section}>
            <Text style={styles.label}>Service Areas * (Select all that apply)</Text>
            <View style={styles.communityGrid}>
              {COMMUNITIES.map((comm) => (
                <TouchableOpacity
                  key={comm}
                  style={[styles.communityItem, serviceAreas.includes(comm) && styles.communityItemActive]}
